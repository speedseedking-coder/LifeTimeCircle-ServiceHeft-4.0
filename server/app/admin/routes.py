...
# Wichtig: SUPERADMIN existiert als Rolle (f�f�'�,¼r High-Risk-Gates)
...
def admin_list_users(_: AuthContext = Depends(require_roles("admin", "superadmin"))):
    """
    Minimaler Admin-�f�'�.�?oberblick: keine Klartext-PII, nur user_id/role/created_at.
    """
...
def admin_create_vip_business(
    ...
    with _connect(settings.db_path) as conn:
        ...
        # Insert (idempotent-ish): wenn exists, liefere bestehenden Zustand zur�f�'�,¼ck
...
def superadmin_add_vip_business_staff(
    ...
    """
    VIP-Gewerbe Staff hinzuf�f�'�?�?T�f�?s�,¼gen:
    - nur SUPERADMIN
    ...
    with _connect(settings.db_path) as conn:
        ...
        # Limit pr�f�'�?�?T�f�?s�,¼fen (max 2 Staff)
        cnt = conn.execute(
            "SELECT COUNT(1) AS c FROM vip_business_staff WHERE business_id=?;",
            (business_id,),
        ).fetchone()["c"]
        if int(cnt) >= 2:
            # keine Zahlen in Fehltexten n�f�'�?�?T�f�?s�,¶tig
