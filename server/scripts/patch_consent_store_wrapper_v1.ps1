...
$target     = Join-Path $serverRoot "app\consent_store.py"

if (-not (Test-Path $target)) { throw "FEHLT: $target" }

Copy-Item $target "$target.bak" -Force

Write-FileUtf8NoBom -Path $target -Content @'
...
Write-Host "OK: app/consent_store.py compat wrapper geschrieben"
